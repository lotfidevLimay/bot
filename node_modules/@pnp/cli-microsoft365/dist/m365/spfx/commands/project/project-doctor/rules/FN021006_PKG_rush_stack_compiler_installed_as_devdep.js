"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.FN021006_PKG_rush_stack_compiler_installed_as_devdep = void 0;
const JsonRule_1 = require("../../JsonRule");
class FN021006_PKG_rush_stack_compiler_installed_as_devdep extends JsonRule_1.JsonRule {
    get id() {
        return 'FN021006';
    }
    get title() {
        return '';
    }
    get description() {
        return '';
    }
    get severity() {
        return 'Required';
    }
    get file() {
        return './package.json';
    }
    get resolutionType() {
        return 'cmd';
    }
    visit(project, findings) {
        if (!project.version ||
            !project.packageJson ||
            !project.packageJson.dependencies) {
            return;
        }
        const projectDeps = Object.keys(project.packageJson.dependencies);
        projectDeps.forEach(dep => {
            if (dep.indexOf('@microsoft/rush-stack-compiler') < 0) {
                return;
            }
            const node = this.getAstNodeFromFile(project.packageJson, `dependencies.${dep}`);
            this.addFindingWithCustomInfo(`${dep} installed as a dependency`, `Package ${dep} is installed as a dependency. Install it as a devDependency instead`, [{
                    file: this.file,
                    resolution: `installDev ${dep}@${project.packageJson.dependencies[dep]}`,
                    position: this.getPositionFromNode(node)
                }], findings);
        });
    }
}
exports.FN021006_PKG_rush_stack_compiler_installed_as_devdep = FN021006_PKG_rush_stack_compiler_installed_as_devdep;
//# sourceMappingURL=FN021006_PKG_rush_stack_compiler_installed_as_devdep.js.map